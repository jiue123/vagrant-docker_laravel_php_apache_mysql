...
if grep -w "DB_HOST=127.0.0.1" $LARAVEL_ENVIRONMENT_FILE; then
  sed -i -e 's/DB_HOST=127.0.0.1/DB_HOST=database/g' $LARAVEL_ENVIRONMENT_FILE
fi

if grep -w "DB_DATABASE=homestead" $LARAVEL_ENVIRONMENT_FILE; then
  sed -i -e 's/DB_DATABASE=homestead/DB_DATABASE=laravel/g' $LARAVEL_ENVIRONMENT_FILE
fi

if grep -w "DB_USERNAME=homestead" $LARAVEL_ENVIRONMENT_FILE; then
  sed -i -e 's/DB_USERNAME=homestead/DB_USERNAME=root/g' $LARAVEL_ENVIRONMENT_FILE
fi

if grep -w "DB_PASSWORD=secret" $LARAVEL_ENVIRONMENT_FILE; then
  sed -i -e 's/DB_PASSWORD=secret/DB_PASSWORD=123456/g' $LARAVEL_ENVIRONMENT_FILE
fi
